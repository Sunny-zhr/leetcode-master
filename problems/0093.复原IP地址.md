...
class Solution:
    def restoreIpAddresses(self, s: str) -> List[str]:
        def isvalid(s):
            assert len(s) <= 3, "Too long string" 
        ...
        ans, path = [], []
        def backtrack(x):
            assert len(path) <= 4, "Too many IPs"
            if len(path) == 4 and sum(map(len,path)) == len(s):
                ans.append(path.copy())
                return
            for i in range(x, min(len(s), x+3)): #剪枝：防止s长度超过3
                if not isvalid(s[x:i+1]): continue #剪枝：非法IP
                if len(path) >= 4: continue #剪枝：防止path数量超过4
                path.append(s[x:i+1])
                backtrack(i+1)
                path.pop()
